...
Begin VB.Form frmTodaysPharmacyList 
   ...
   End
End
Attribute VB_Name = "frmTodaysPharmacyList"
...
Private Sub btnIndoor_Click()
    ...
    With rsTemItem
        If .State = 1 Then .Close
        temSql = "SELECT tblVisit.VisitID, tblVisit.PatientID, tblVisitItem.PItemID, tblVisitItem.PIssueQuentity " & _
        ...
        temSql = "SELECT tblVisit.VisitID, tblVisit.PatientID, tblVisitItem.PItemID, tblVisitItem.PIssueQuentity " & _
                    ...
                    "WHERE tblVisit.VisitID = " & Val(lstQueue.BoundText) & " AND tblVisitItem.Indoor = true AND tblVisitItem.Deleted = False"
        
        .Open temSql, cnnStores, adOpenStatic, adLockReadOnly
        If .RecordCount > 0 Then
            MyVisitID = !VisitID
            frmIndoorIssue.cmbCustomer.BoundText = !PatientID
            While .EOF = False
    ...
    With rsQueue
        If .State = 1 Then .Close
        temSql = "Select * from tblVisit where VisitID = " & Val(lstQueue.BoundText)
        .Open temSql, cnnStores, adOpenStatic, adLockOptimistic
        If .RecordCount > 0 Then
            !IndoorIssued = True
...
Private Sub lstQueue_Click()
    ...
    With rsTemItem
        ...
        temSql = "SELECT tblItem.Item, tblVisitItem.PItemID, tblVisitItem.PIssueQuentity FROM (tblVisit LEFT JOIN tblVisitItem ON tblVisit.VisitID = tblVisitItem.VisitID) LEFT JOIN tblItem ON tblVisitItem.PItemID = tblItem.ItemID " & _
                    "WHERE (((tblVisitItem.PItemID)<>0) AND ((tblVisit.VisitID)=" & Val(lstQueue.BoundText) & ") "
                    
        If optToIndoorIssue.Value = True Then
            temSql = temSql & " AND ((tblVisitItem.Indoor)=True) "
        ElseIf optToOutdoorIssue.Value = True Then
            temSql = temSql & " AND ((tblVisitItem.Indoor)=False) "
        End If
        temSql = temSql & " AND ((tblVisitItem.Deleted)=False))"
