...
Begin VB.Form frmStockAdjustment 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "frmStockAdjustment.frx":0000
      ...
      TabPicture(1)   =   "frmStockAdjustment.frx":001C
   ...
   Begin MSDataListLib.DataCombo cmbCode 
      ...
      Text            =   ""
   End
   Begin MSDataListLib.DataCombo cmbBatch 
      Height          =   360
      Left            =   2160
      TabIndex        =   9
      Top             =   2040
...
Private Sub GetFormSettings()
    ...
    cmbDepartment.BoundText = UserDepartmentID
End Sub
...
Private Sub btnSave_Click()
    ...
    If ChangeStocks(Val(cmbItem.BoundText), Val(cmbBatch.BoundText), Val(txtINewStock.Text), Val(cmbDepartment.BoundText), False, False, True) = True Then
        With rsTem
            ...
            !ActionID = UserAction.StockReset
            !ActionDate = Date
            !ActionTime = Time
            !ActionDateTime = Now
            !RefID = Val(cmbItem.BoundText)
            !RefSubID = Val(cmbBatch.BoundText)
    ...
    Else
        MsgBox "Error, Stocks NOT updated"
    End If
    Call ClearValues
...
Private Sub DisplayBatch()
    ...
    If IsNumeric(cmbBatch.BoundText) = False Then Exit Sub
    txtICurrentStock.Text = CalculateStock(Val(cmbItem.BoundText), Val(cmbBatch.BoundText), Val(cmbDepartment.BoundText)).Stock
End Sub

Private Sub cmbBatch_Click(Area As Integer)
    Call DisplayBatch
End Sub
...
Private Sub cmbItem_Change()
    ...
    With rsBatch
        If .State = 1 Then .Close
        temSql = "Select * from tblItemBatch where ItemID = " & Val(cmbItem.BoundText) & " AND Deleted = False order by ItemBatch"
        .Open temSql, cnnStores, adOpenStatic, adLockReadOnly
    End With
    With cmbBatch
