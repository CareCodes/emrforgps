...
Begin VB.Form frmPharmacyQueueIncome 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Pharmacy Secession Income"
   ClientHeight    =   8460
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   7365
   ...
   End
End
Attribute VB_Name = "frmPharmacyQueueIncome"
...
'    If IsNumeric(cmbSecession.BoundText) = False Then Exit Sub
    ...
    With rsTem
        ...
        If IsNumeric(cmbDoctor.BoundText) = True And IsNumeric(cmbSecession.BoundText) = True Then
            temSql = "SELECT tblSaleBill.*, tblPatient.Patient, tblQueue.* " & _
                        "FROM (tblQueue LEFT JOIN tblSaleBill ON tblQueue.QueueID = tblSaleBill.QueueID) LEFT JOIN tblPatient ON tblQueue.PatientID = tblPatient.PatientID " & _
        ...
        ElseIf IsNumeric(cmbDoctor.BoundText) = False And IsNumeric(cmbSecession.BoundText) = True Then
            temSql = "SELECT tblSaleBill.*, tblPatient.Patient, tblQueue.* " & _
                        "FROM (tblQueue LEFT JOIN tblSaleBill ON tblQueue.QueueID = tblSaleBill.QueueID) LEFT JOIN tblPatient ON tblQueue.PatientID = tblPatient.PatientID " & _
                        "WHERE (((tblQueue.QueueDate)=#" & Format(dtpDate.Value, "dd MMMM yyyy") & "#) AND ((tblQueue.SecessionID)= " & Val(cmbSecession.BoundText) & "))"
        
        ElseIf IsNumeric(cmbDoctor.BoundText) = True And IsNumeric(cmbSecession.BoundText) = False Then
            temSql = "SELECT tblSaleBill.*, tblPatient.Patient, tblQueue.* " & _
                        "FROM (tblQueue LEFT JOIN tblSaleBill ON tblQueue.QueueID = tblSaleBill.QueueID) LEFT JOIN tblPatient ON tblQueue.PatientID = tblPatient.PatientID " & _
                        "WHERE (((tblQueue.DoctorID)=" & Val(cmbDoctor.BoundText) & ") AND ((tblQueue.QueueDate)=#" & Format(dtpDate.Value, "dd MMMM yyyy") & "#))"
        
        ElseIf IsNumeric(cmbDoctor.BoundText) = False And IsNumeric(cmbSecession.BoundText) = False Then
            temSql = "SELECT tblSaleBill.*, tblPatient.Patient, tblQueue.* " & _
                        "FROM (tblQueue LEFT JOIN tblSaleBill ON tblQueue.QueueID = tblSaleBill.QueueID) LEFT JOIN tblPatient ON tblQueue.PatientID = tblPatient.PatientID " & _
                        "WHERE (((tblQueue.QueueDate)=#" & Format(dtpDate.Value, "dd MMMM yyyy") & "#))"
        ...
        While .EOF = False
            ...
            GridIncome.Col = 2
            If IsNull(!BillTime) = False Then
                GridIncome.Text = Format(![BillTime], "HH:MM AMPM")
            End If
