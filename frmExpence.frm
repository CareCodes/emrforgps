...
Private Sub btnAdd_Click()
    ...
    With rsExpence
        ...
        !ExpenceMethodID = ExpenceMethod.OtherExpence
        !ExpenceTime = Time
        !ExpenceDateTime = dtpDate.Value + Time
        !AddedDate = Date
        !AddedUserID = UserID
        !AddedTime = Time
        !AddedDateTime = Now
    ...
    temBool = ChangeDrawerBalances(Val(cmbDrawer.BoundText), Val(cmbPaymentMethod.BoundText), Val(txtValue.Text), False, True, False)
    Call ClearAddValues
    Call FormatGrid
    Call FillGrid
    cmbCategory.SetFocus
...
Private Sub btnDelete_Click()
    ...
    With rsExpence
        ...
        If .RecordCount > 0 Then
            ...
            temBool = ChangeDrawerBalances(!DrawerID, !PaymentMethodID, !Value, True, False, False)
            .Update
        End If
        .Close
    End With
...
Private Sub FillCombos()
    ...
    Cat.FillBoolCombo cmbCategory, "Category", "Category", "IsExpenceCategory", True
...
Private Sub FillGrid()
    ...
    With rsExpence
        If .State = 1 Then .Close
        temSql = "SELECT tblExpence.*, tblCategory.Category FROM tblExpence LEFT JOIN tblCategory ON tblExpence.ExpenceCategoryID = tblCategory.CategoryID where tblExpence.ExpenceDate = #" & Format(dtpDate.Value, "dd MMMM yyyy") & "# And tblExpence.DepartmentID = " & Val(cmbDepartment.BoundText) & " And tblExpence.Deleted = false  AND ExpenceMethodID = " & ExpenceMethod.OtherExpence
        ...
        If .RecordCount > 0 Then
            ...
            For i = 1 To .RecordCount
                ...
                gridExpence.TextMatrix(i, 1) = Format(!Category, "")
                gridExpence.TextMatrix(i, 2) = Format(!Comments, "")
                gridExpence.TextMatrix(i, 3) = Format(!Value, "#,##0.00")
                MyTotal = MyTotal + !Value
                gridExpence.TextMatrix(i, 4) = !ExpenceID
                gridExpence.row = i
                If gridExpence.row Mod 2 = 0 Then
                    gridExpence.CellBackColor = DefaultColourScheme.GridLightBackColour
                Else
                    gridExpence.CellBackColor = DefaultColourScheme.GridDarkBackColour
                End If
                .MoveNext
            Next
        End If
